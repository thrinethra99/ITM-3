...
<body>
	...
	$(window).on('load', function() {
		...
		function initArea(){
			$("#area").empty().html(' ');
			var initHtml =  '<optgroup label="Options"><option value="options">options</option></optgroup>'
		  ...
		  $('select').on('contentChanged', function() {
			...
			function project_wondering(project_id,wid){
				...
				$(document).ajaxStop(function() {
					if(project_id!=-1){
						$.ajax({
					        ...
					        type: 'POST',
					        data: {'projectid': project_id,'projectname':$("#title").val()},
					        success: function (response) {
					        	// change save into editor mode, the solution for edit is delete whole project inform then add as a new project . 
					        	$("#create-submit").data("mode",project_id)
					        	project_id=-1
					        	$(".after-save").show()
					        }
						});
				...
				$("#btn-back-project-list").click(function(){
				       window.location.href = "../openedit.jsp";
				})
