				...
				<div class="container">
					...
					function GetSelectedTextRed(){
						 ...
						 if(sel != 0){
						    ...
						    document.designMode = "on";
						    document.execCommand("BackColor", false, "#ff5b5b");
						    document.designMode = "off";
						 }
							else{
								Materialize.toast('Please select text to highlight!', 4000)
					...
					<ul id='highlightOptions' class='dropdown-content collection' style="width:100px;">
						...
						<li style="background-color:#fdff4a"><a onmousedown="GetSelectedTextYellow()" href="#!"><span style="font-size: 11.5px;color:black;" title="A “juicy” question for research"><b>A “juicy” question for research</b></span></a></li>
						<li style="background-color:#72e567"><a onmousedown="GetSelectedTextGreen()" href="#!"><span style="font-size: 11.5px;color:black;" title="An “Aha” insight"><b>An “Aha” insight</b></span></a></li>
						<li style="background-color:#fdb760"><a onmousedown="GetSelectedTextOrange()" href="#!"><span style="font-size: 11.5px;color:black;" title="A seed idea to refine"><b>A seed idea to refine</b></span></a></li>
						<li style="background-color:#a6acff"><a onmousedown="GetSelectedTextBlue()" href="#!"><span style="font-size: 11.5px;color:black;" title="An important fact to consider"><b>An important fact to consider</b></span></a></li>
						<li style="background-color:#ff5b5b"><a onmousedown="GetSelectedTextRed()" href="#!"><span style="font-size: 11.5px;color:black;" title="Conflicting ideas to look into"><b>Conflicting ideas to look into</b></span></a></li>
						<li style="background-color:#c0b1d0"><a onmousedown="GetSelectedTextPurple()" href="#!"><span style="font-size: 11.5px;color:black;" title="A gap of knowledge"><b>A gap of knowledge</b></span></a></li>
								...
								var title=$('#note_title').val();
							 	...
							 	var data = {
							 			project_id: parseInt($('#projectid').text()),
						   	    	    ...
						   	    	    views: views,
						   	    	    scaffolds: scaffolds,
						   	    	    plain_content:plain_content.replace(/'/g,"\\\'").replace(/"/g,"\\\"")
						   	    };
				   	  			$.ajax({
					   	    	    type: "POST",
					   	    	    async: true,
	...
	<script>
		...
		$.ajax({
			...
			success : function(data) {
				...
				var tester_height= json.length*21;
				$("#tester").height(tester_height);
				}
				...
				$.ajax({
					...
					success : function(data) {
						...
						if(json!=null){
							...
							$('#showbuildon').attr('checked', true);
							//Materialize.toast('No buildon note(s) available for this thread.',4000);
						}
		...
		function add(){
			 ...
			 var json = {
					 "database" :'<%=session.getAttribute("localdb")%>' ,
					 "token" : '<%=session.getAttribute("token")%>',
					 "title":$("#add_note_title").val().replace(/'/g,"\\\'").replace(/"/g,"\\\""),
					 "noteid":nid,
					 "content":tinyMCE.activeEditor.getContent().replace(/'/g,"\\\'").replace(/"/g,"\\\""),
					 "ctime":new Date(),
					 "offset":"-1"
			 }
			 $.ajax({
	  ...
	  $('select').on('contentChanged', function() {
		...
		$(".jofsave").click(function() {
			...
			var jsondata = {
				...
				"token" : '<%=session.getAttribute("token")%>',
				"threadid" : threadid,
				"ourproblem":$("#problem_textarea").val().replace(/'/g,"\\\'").replace(/"/g,"\\\""),
				"createtime":new Date(),
				"authorid":'<%=session.getAttribute("id")%>',
				"bigidea" : $("#idea_textarea").val().replace(/'/g,"\\\'").replace(/"/g,"\\\""),
				"more" : $("#to_do_textarea").val().replace(/'/g,"\\\'").replace(/"/g,"\\\"")
			};
			$.ajax({
				url :  "/WSG/jot/add",
				type : "POST",
