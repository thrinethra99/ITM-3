...
<main>
							...
							<table>
								<thead>
									<tr>
										<th>Cross-project
									Sharing Space</th>
										<th><a class="btn"  style="height:20px;line-height:20px" href="../../thread/cross_project_sharing.jsp">Enter</a></th>
									</tr>
								</thead>
	...
	$("#mapping_finish").click(function(){
		...
		if($("#tbox p input").prop('checked') == true && $("#wbox p input").prop('checked') == true){
		...
		else if($("#tbox p input").prop('checked') == true){
			$('#tbox p input').each(function() {
			    if($(this).is(':checked')){
			    	tboxcheckedtxt.push($($($(this).parent()).children()[1]).text());
			    	tboxchecked.push($(this).attr('id').replace("mthread", ""));
		...
		else if($("#wbox p input").prop('checked') == true){
			...
			$("#btn-add-teacher-reminder").click(function(){
				...
				$("#add_reminder").modal('open');
								...
								function initTeacherReminder(){
									...
									$("#page_title").html('<a href="../../dashboard.jsp"><%=session.getAttribute("community")%></a>-><a href="../openedit.jsp">'+project.title+'</a>')
									$('select').material_select();
									function init(){
										initProjectGoal()
										initTeacherReminder()
