...
<div class="row">
	...
	<div class="col-lg-1 col-md-12 col-sm-12 col-xs-12">
		...
		<div class="col-lg-1 col-md-12 col-sm-12 col-xs-12">
			<a href="thread_import.jsp" id="import" target="_self" class="waves-effect waves-light btn">Import</a>
		</div>
		
		<div class="col-lg-1 col-md-12 col-sm-12 col-xs-12 file-field">
			<div class="btn"
    	...
    	<div class="col-lg-1 col-md-12 col-sm-12 col-xs-12 click-to-toggle jotdetail"  id="jofedit">
		<a class="btn  click-to-toggle jotdetail"  id="jofedit"> Journey of Thinking </a>
		</div>
</div>
</div>
		<script>
...
<div id="googledoc" class="modal">
	...
	<script>
		...
		function get_community_info(call_back){
			...
			if(null!=getUrlParameter("c_id")){
				var userdata = {
						"database" : "itm3",
						"token" : token,
						"cid" : getUrlParameter("c_id")
					};
				$.ajax({
							...
							$.ajax({
										...
										success : function(data, textStatus,jqXHR) {
											...
											if(null!=json){
												...
												if(isCrossProject){
													var html2 ="<a href='../dashboard.jsp'>[CROSSPROJECT]"+community+"</a>-><a href='../project/openedit.jsp'>"+project_name+"</a> >"
													...
													var html2 ="<a href='../dashboard.jsp'>"+community+"</a>-><a href='../project/openedit.jsp'>"+project_name+"</a> >"
													var html =' <div style="float:left"><select id="thread_list">'	
													for (var i in json) {
														if(json[i].thread_id!=threadid){
															html=html+'  <option value="'+json[i].thread_id+'" style="width: 400px;">'+json[i].threadfocus+'</option>'
									...
									$.ajax({
										...
										data : {
											...
											success : function(response) {
												window.location.href = "thread.jsp?localdb="+localdb+"&token="+token+"&community="+community+"&community_id="+community_id+"&type="+type+"&urname="+urname+"&id="+id+"&itmdb="+itmdb+"&domain="+domain+"&school="+school+"&country="+country+"&city="+city+"&projectid="+projectid+"&threadid="+$("nav select option:selected" ).val()+"&threadname="+$("nav select option:selected" ).text();
											}
										});
	...
	<script>
		...
		var stripes_colors=["#FFDAB9","#BDB76B","#FFE4B5","#B0A6A4","#F5F5DC"]
		var stripeColor = '';
		...
		function add(){
			 var nid =projectid+threadid+Math.floor((Math.random() * 10000) + 1).toString();
			 var json = {
					 "database" :localdb ,
					 "token" : token,
