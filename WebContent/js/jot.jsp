...
<script type="text/javascript">
		...
		websocket.onmessage = function(event) {
			...
			if(	
					data.database=='<%=session.getAttribute("localdb")%>'
					&&data.thread==<%=session.getAttribute("threadid")%>
					&&data.project==<%=session.getAttribute("projectid")%>
			){
		...
		function saveJot(cate,data) {
			...
			if(cate=="problem"){
				jsondata.ourproblem=data.content
				
			}
			if(cate=="idea"){
				jsondata.bigidea=data.content
			}
			if(cate=="more"){
				jsondata.more=data.content
			}
	  ...
	  function jotUpdate(cate,id,content){
			var jsondata={
					"database" :'<%=session.getAttribute("localdb")%>' ,
					"token" : '<%=session.getAttribute("token")%>',
					"mtime":new Date(),
					"editorId":'<%=session.getAttribute("id")%>',
					"ourproblem":"",
					"bigidea":"",
					"more":"",
					"jotId":id
		  ...
		  if(cate == "problem_textarea"){
			  jsondata["ourproblem"]=content
		  }
		 if(cate == "idea_textarea"){
			 jsondata["bigidea"]=content
		  }
		 if(cate == "to_do_textarea"){
			 jsondata["more"]=content
		 }
		...
		var jot_websocket_data ={
				...
				'database':'<%=session.getAttribute("localdb")%>',
				'project' :<%=session.getAttribute("projectid")%>,
				'thread' :<%=session.getAttribute("threadid")%>,
				'cate' : '',
				'action' : 'add',
